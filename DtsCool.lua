if game.Players:FindFirstChild("trymology") then
_G.Settings = {
    AimEnable = false,
    EspEnable = false,
    AimAutoShoot = false,
...
local SirhurtFix = Vector2.new(0, 0)

--Shitty sirhurt fix.
if is_sirhurt_closure then
    SirhurtFix = Vector2.new(12, 0)
end
...
local function isVisible(character)
    local Ray = Ray.new(Camera.CFrame.p, (character[_G.Settings.AimPart].Position - Camera.CFrame.p).unit * 2048)
...
local function Calculate(distance)
    local aimup = (1.01*math.pow(distance,4)+0.002*math.pow(distance,3)-0.07*math.pow(distance,2)+0.8*distance-2.05)
    return aimup
end
...
local function IsInFov(part)
    if part then
        if IsOnScreen(part) then
            ...
            if dist <= workspace.CurrentCamera.ViewportSize.X / (90 / _G.Settings.AimFOV) and dist < math.huge then
                return true
            end
        end
    end
...
local function GetClosestNotBehindWall()
    local TargetDistance = math.huge
    local Target = nil
    for i, v in pairs(Players:GetPlayers()) do
        if v ~= LocalPlayer and game.workspace:FindFirstChild(v.Name) and game.workspace:FindFirstChild(v.Name):FindFirstChild("HumanoidRootPart") then
            if game.workspace:FindFirstChild(v.Name):FindFirstChild("Humanoid").Health > 0 then
            local mousePos = WorldToScreen(Mouse.Hit.p)
            local TargetScreenPos = workspace.CurrentCamera:WorldToScreenPoint(game.workspace:FindFirstChild(v.Name).HumanoidRootPart.Position)
            local mag = (Vector2.new(TargetScreenPos.X, TargetScreenPos.Y) - Vector2.new(mousePos.X, mousePos.Y)).magnitude
            if IsInFov(game.workspace:FindFirstChild(v.Name):FindFirstChild(_G.Settings.AimPart)) or IsInFov(game.workspace:FindFirstChild(v.Name):FindFirstChild("HumanoidRootPart")) then
                if _G.Settings.AimTeamCheck then
                    if v.TeamColor ~= LocalPlayer.TeamColor then
                        if _G.Settings.AimWallCheck then
                            if isVisible(game.workspace:FindFirstChild(v.Name)) and mag < TargetDistance then
                            	if game.workspace:FindFirstChild(v.Name).Torso.Transparency == 0 then
                                TargetDistance = mag
                                Target = game.workspace:FindFirstChild(v.Name)
                            end
                            end
                        end
                    end
                elseif _G.Settings.AimWallCheck then
                    if isVisible(game.workspace:FindFirstChild(v.Name)) and mag < TargetDistance then
                    	if game.workspace:FindFirstChild(v.Name).Torso.Transparency == 0 then
                        TargetDistance = mag
                        Target = game.workspace:FindFirstChild(v.Name)
...
local function aimbot()
    if FindClosest ~= nil then
        local mousePos = WorldToScreen(Mouse.Hit.p)
        local aimAt, visible = game.Workspace.CurrentCamera:WorldToScreenPoint(FindClosest[_G.Settings.AimPart].Position + Vector3.new(0,Calculate((game.workspace:FindFirstChild(game.Players.LocalPlayer.Name).HumanoidRootPart.Position - FindClosest.HumanoidRootPart.Position).magnitude / _G.Settings.AimBulletDrop) / _G.Settings.AimBulletDrop,0)) --We get the target's Torso and add a new vector3 with y being BulletDrop, then we get that from vector3 to vector2, and we basically get the magnitude of target and localplayer and then use our sick math to get a nice curve.
        mousemoverel((aimAt.X - mousePos.X) / _G.Settings.AimSmoothness, (aimAt.Y - mousePos.Y) / _G.Settings.AimSmoothness)
    end
end
...
NumSides = 16,
Filled = false,
Color = _G.Settings.FOVColor,
Visible = false
});

game:GetService("RunService").RenderStepped:connect(function()
    local mousePos2 = WorldToScreen(Mouse.Hit.p)
    drawings.Circle.Position = Vector2.new(mousePos2.X, mousePos2.Y + 36)
    if IsKeyDown then
        wait() --This is just a bad fix for my unoptimized code.
        if game.PlaceId == 292439477 then
            if game.Workspace.Players.Ghosts:FindFirstChild(FindClosest.Name) or game.Workspace.Players.Phantoms:FindFirstChild(FindClosest.Name) then --This makes sure that the targetted player is actually alive.
                aimbot()
            end
        else
            aimbot()
			...
			function window_data:AddTab(tab_name)
				...
				do -- Tab Elements
					...
					function tab_data:AddKeybind(keybind_name, callback, keybind_options)
						...
						keybind_options = {
							["standard"] = keybind_options.standard or Enum.UserInputType.MouseButton2,
						}
						...
						input.MouseButton1Click:Connect(function()
							...
							keybind_data:SetKeybind(a.UserInputType)
						end)
...
do -- Example UI
	local Window = library:AddWindow("FFckr V1.02 - envo#0001", {
		main_color = Color3.fromRGB(200, 50, 50),
		min_size = Vector2.new(372, 500),
		toggle_key = Enum.KeyCode.RightShift,
		can_resize = true,
	})
	local Tab = Window:AddTab("Gun Mods")
	local Slider1 = Tab:AddSlider("Ammo Clip", function(x)
		print(x)
	end, {
			["min"] = 30,
			["max"] = 1000,
	...
	local Slider2 = Tab:AddSlider("Reload Time", function(x)
		print(x)
	end, {
			["min"] = 0,
			["max"] = 5,
	...
	local Slider3 = Tab:AddSlider("Fire Rate", function(x)
		print(x)
	end, {
			["min"] = 0,
			["max"] = 100,
	...
	local Slider4 = Tab:AddSlider("Spread", function(x)
		print(x)
	end, {
			["min"] = 0,
			["max"] = 100,
	...
	local Slider5 = Tab:AddSlider("Ray Offset", function(x)
		print(x)
	end, {
			["min"] = 0,
			["max"] = 100,
	...
	Slider4:Set(0)
	Slider5:Set(0)
    local Tab2 = Window:AddTab("Esp Config")
    local switch = Tab2:AddSwitch("ESP : ON/OFF", function(bool)
	on = false
	_G.Settings.EspEnable = bool
	if bool == true then
	if _G.Settings.EspEnable == true then
    for i,v in pairs(game.Players:GetChildren()) do
    if game.workspace:FindFirstChild(v.Name) ~= game.workspace:FindFirstChild(game.Players.LocalPlayer.Name) and game.workspace:FindFirstChild(v.Name).Head:FindFirstChild('ScreenGui') == nil then
        if game.workspace:FindFirstChild(v.Name):FindFirstChild('Head') then
            local bill = Instance.new('BillboardGui',game.workspace:FindFirstChild(v.Name).Head)
            bill.Name = "thingyye"
            bill.AlwaysOnTop = true
            bill.Size = UDim2.new(2,1,2)
            if game.workspace:FindFirstChild(v.Name).Torso.Transparency =="1" then
            bill.Enabled = false
            end
            bill.Adornee = game.workspace:FindFirstChild(v.Name).Head 
        ...
        for a,c in pairs(game.workspace:FindFirstChild(v.Name):GetChildren()) do
            if c.ClassName == "MeshPart" and c.Transparency ~= 1 then
                doit(c,v.TeamColor.Color)
    ...
    on = false
    for i,v in pairs(game.Players:GetChildren()) do
        undo(game.workspace:FindFirstChild(v.Name))
    end
    end
    for i,v in pairs(game.Players:GetChildren()) do
    ...
    v.CharacterAdded:connect(function()
    game.workspace:FindFirstChild(v.Name):WaitForChild('Head')
    wait(1)
    if bool == true then
    if _G.Settings.EspEnable == true then
    if game.workspace:FindFirstChild(v.Name) ~= game.workspace:FindFirstChild(game.Players.LocalPlayer.Name) and game.workspace:FindFirstChild(v.Name).Head:FindFirstChild('ScreenGui') == nil then
    ...
    while true do
    for i,xvf in pairs(game.Players:GetChildren()) do
    if game.workspace:FindFirstChild(xvf.Name):WaitForChild("Torso").Transparency == 1 then
    for i,bfv in pairs(game.workspace:FindFirstChild(xvf.Name):GetDescendants()) do
    if bfv:IsA("BillboardGui") then
    v.Enabled = false
    end
    end
    end
    ...
    local Tab3 = Window:AddTab("Aimbot Config")
    local switch51 = Tab3:AddSwitch("AIMBOT : ON/OFF", function(bool)
    _G.Settings.AimEnable = bool
    end)
    local switchehee = Tab3:AddSwitch("FOV CIRCLE : ON/OFF", function(bool)
    drawings.Circle.Visible = bool
    end)
    local egewgewgweg = Tab3:AddSwitch("TEAM CHECK : ON/OFF", function(bool)
    _G.Settings.AimTeamCheck = bool
    end)
    local egewgewgweg = Tab3:AddSwitch("WALL CHECK : ON/OFF", function(bool)
    _G.Settings.AimWallCheck = bool
    end)
    Tab3:AddLabel("FOV Radius Color")
    local cp = Tab3:AddColorPicker(function(color)
	drawings.Circle.Color = color
    end)
    local slid = Tab3:AddSlider("FOV Size", function(x)
    	local value = x*0.1
    	drawings.Circle.Radius = workspace.CurrentCamera.ViewportSize.X / (90 / value)
		_G.Settings.AimFOV = value
	end, {
			["min"] = 1,
			["max"] = 100,
			["readonly"] = false,
	...
	slid:Set("50")
	local srtr = Tab3:AddSlider("Aimlock Smoothness", function(x)
    	local value = x
		_G.Settings.AimSmoothness = value
	end, {
			["min"] = 1,
			["max"] = 10,
			["readonly"] = false,
	...
	srtr:Set("3")
	local Dropdown21 = Tab3:AddDropdown("Aim Part", function(text)
	if text == "Head" then
		_G.Settings.AimPart = "Head"
	elseif text == "Torso" then
		_G.Settings.AimPart = "Torso"
	end
    end)
    local firstone = Dropdown21:Add("Head")
    local secondone = Dropdown21:Add("Torso")
    local Dropdown212 = Tab3:AddDropdown("Toggle Keybind", function(text)
	if text == "Left Mouse" then
		_G.Settings.AimAimKey = Enum.UserInputType.MouseButton1
	elseif text == "Right Mouse" then
		_G.Settings.AimAimKey = Enum.UserInputType.MouseButton2
	end
    end)
    local safawefgeawg = Dropdown212:Add("Left Mouse")
    local shtrjytktuyh = Dropdown212:Add("Right Mouse")
    local hjtrjrtyjrtj = Dropdown212:Add("Left Ctrl")
    local sdgerhtrettj = Dropdown212:Add("Left Shift")
    local Tab4 = Window:AddTab("Others")
    local switch2 = Tab4:AddSwitch("TRIGGERBOT : ON/OFF", function(bool)
    ...
    local switch2 = Tab4:AddSwitch("SPINBOT : ON/OFF", function(bool)
    if bool == true then
    if game.workspace:FindFirstChild(game.Players.LocalPlayer.Name).HumanoidRootPart:FindFirstChild("BodyThrust") then
    game.workspace:FindFirstChild(game.Players.LocalPlayer.Name).HumanoidRootPart:FindFirstChild("BodyThrust").Force = Vector3.new(_G.Settings.SpinPower,0,_G.Settings.SpinPower)
    else
    local bambam = Instance.new("BodyThrust")
    bambam.Parent = game.workspace:FindFirstChild(game.Players.LocalPlayer.Name).HumanoidRootPart
    bambam.Force = Vector3.new(_G.Settings.SpinPower,0,_G.Settings.SpinPower)
    bambam.Location = game.workspace:FindFirstChild(game.Players.LocalPlayer.Name).HumanoidRootPart.Position
    end
    elseif bool == false then
    game.workspace:FindFirstChild(game.Players.LocalPlayer.Name).HumanoidRootPart:FindFirstChild("BodyThrust"):Remove()
    end
    end)
    local slidererr = Tab4:AddSlider("Camera FOV", function(x)
		game.workspace:FindFirstChild("Camera").FieldOfView = x
	end, {
			["min"] = 70,
			["max"] = 120,
			["readonly"] = false,
	...
	slidererr:Set("0")
	local wfefwgweg = Tab4:AddSlider("SpinBot Power", function(x)
		_G.Settings.SpinPower = x
		if game.workspace:FindFirstChild(game.Players.LocalPlayer.Name).HumanoidRootPart:FindFirstChild("BodyThrust") then
        game.workspace:FindFirstChild(game.Players.LocalPlayer.Name).HumanoidRootPart:FindFirstChild("BodyThrust").Force = Vector3.new(_G.Settings.SpinPower,0,_G.Settings.SpinPower)
        end
	end, {
